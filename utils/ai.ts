...
const parser = StructuredOutputParser.fromZodSchema(
    ...
    sentimentScore: z.number().describe('sentiment of the text and rated on a scale from -10 to 10, where -10 is extremely negative, 0 is neutral, and 10 is extremely positive.'),
        ...
        summary: z.string().describe('short summary of the whole entry.'),
        subject: z.string().describe('short subject of the entry'),
        negative: z.boolean().describe('is the journal entry negative? (i.e. does it contain negative emotions?'),
        color: z.string().describe('hexidecimal color code that represents the  mood of the entry. Example #33f507 for green for happiness'),
    })
)
...
export const analyze = async (content) => {
    ...
    const model = new OpenAI({temperature:0, modelName:'gpt-3.5-turbo'}) //more temperature more silly the ai :)
